...
<DefinitionList>
  ...
  <dd>Im unten stehenden Beispielprogramm definiert die Funktion `zeichne_quadrat()` auf Zeile `3` einen _Parameter_ namens `seitenlaenge`.</dd>
  <dd>Sie nimmt beim Aufruf also einen Wert entgegen, den sie (automatisch) der Variable `seitenlaenge` zuweist.</dd>
  <dt>Argument</dt>
  <dd>Ein _Argument_ ist ein konkreter Wert, den wir einer Funktion beim Aufruf für einen bestimmten Parameter mitgeben.</dd>
  <dd>Im unten stehenden Beispielprogramm wird die `zeichne_quadrat()`-Funktion auf Zeile `8` mit dem _Argument_ `100` für den Parameter `seitenlaenge` aufgerufen.</dd>
  <dd>Auf Zeile `9` wird sie erneut aufgerufen — diesmal jedoch mit dem _Argument_ `150` für den Parameter `seitenlaenge`.</dd>
</DefinitionList>
:::
