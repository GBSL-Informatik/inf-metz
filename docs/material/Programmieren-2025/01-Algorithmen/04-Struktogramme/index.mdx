...
:::tip[Farben]
Die hier verwendeten Farben sind nur eine optische Hilfe. Sie haben keine besondere Bedeutung.
:::
...
:::tip[Bemerke]
Beim Struktogramm wird der wiederholte Teil durch die **Einrückung** ersichtlich. Auch beim Programmieren in vielen Programmiersprachen werden wiederholte Sequenzen visuell mit einem Tabulator eingerückt.
...
Mit den vertikalen Balken zeigen wir, dass es sich um einen Unterprogramm-Aufruf handelt. Nur so dürfen wir die Befehle `Spaghetti(…)` und `Tomatensauce(…)` verwenden. Wir können sie nicht einfach so in einem Struktogramm schreiben, sondern müssen sie immer in einem Unterprogramm-Aufruf verwenden – denn sonst wäre nicht klar, was dort genau gemacht werden soll.
...
Das folgende Programm `Addieren(a, b)` addiert zwei Zahlen und gibt das Ergebnis zurück:
...
<Struktogramm program={[
  ...
  {type: 'output', code: <span>Rückgabe: <span className="var">c</span></span>}
]} />

Wir können seinen **Rückgabewert** nun in einem anderen Programm, z.B. `Mittelwert(a, b)`, verwenden:
...
Variablen werden vom Algorithmus bei der Ausführung mit Werten gefüllt, um diese bei der Berechnung zu verwenden oder um sie später auszugeben. Bei einigen Variablen können wir den Wert aber auch von aussen vorgeben – nämlich immer bei einem solchen Block:

<Struktogramm program={[
  {type: 'input', code: <span>Eingabe: <span className="var">x</span></span>}
]} />

Dieser Block erlaubt uns, den Wert eine Variable – in diesem Fall `x` – von aussen vorzugeben. Eine solche Variable nennen wir einen **Parameter**. _Parameter_ sind die **E**ingabe im **EVA**-Prinzip. Wir können damit steuern, mit welchem Wert (oder welchen Werten) der Algorithmus ausgeführt werden soll.
